...
def batchDescent(W, X, Y, bsize, n, t, mode):
    
    if mode == 1:
        for j in range(t):
            i = 0
            while i<X.shape[0]:
                W = Gradient(W, X[i:i+bsize, :], Y, n, 1)
                i = i + bsize
        return W
...
if(te == 1):
    ...
    bsize = f1.readline()
    bsize = int(bsize)
    W = batchDescent(W, xtrain, ytrain, bsize, lrate, it, 1)
...
#Adaptive learning rate
    ...
    it = f1.readline()
    it = int(it)
    W = GDescentAdaptive(W, xtrain, ytrain, lrate, it)
    ...
    it = f1.readline()
    it = int(it)
    W = BackGradient(W, xtrain, ytrain, n, it, alpha, beta)
