...
#     {"layer_size": 2000, "activation_function": "Sigmoid"},
#     {"layer_size": 1000, "activation_function": "Relu"},
...
nn_architecture = [
    {"layer_size": 1024, "activation_function": "Sigmoid"},      #Input Layer
    {"layer_size": 500, "activation_function": "Relu"},
    {"layer_size": 13, "activation_function": "Relu"},
    {"layer_size": 10, "activation_function": "Softmax"}      #Output Layer
]
...
def n_network(X, Y, nn_architecture, lrate, para):
    AL, temp = forward_propagation(X, nn_architecture, para)
    grad = backward_propagation(AL, Y, nn_architecture, para, temp)
    para = update_para(para, grad, lrate)
    return para
...
def Accuracy(pred, Y):
    ...
    for i in range(Y.shape[0]):
        if(pred[i] == Y[i]):
            count = count + 1
    acc = (count*100)/(Y.shape[0])
    return acc

# #Finally Calling the function and integrating the values here
# learning_rate = np.array([0.001, 0.01, 0.7, 0.3, 1.0, 3.0, 10.0, 100.0, 1000.0])
...
#                 Yt = Y[j*bsize:len(X)]
#             else:
#                 Xt = X[j*bsize:(j+1)*bsize]
#                 Yt = Y[j*bsize:(j+1)*bsize]
#             para = n_network(Xt, Yt, nn_architecture, lt, para)
#     AL, temp = forward_propagation(X, nn_architecture, para) 
#     AL = AL.T
...
#     pred = pred[1:]
#     #print(ty.shape, pred.shape)

#     acc = Accuracy(pred, ty)
#     accuracy.append(acc)
#     print(acc, learning_rate[l])
#     if max<acc:
#         max = acc
#         lr = learning_rate[l]
#         pr = para

# plt.plot(np.squeeze(learning_rate), np.squeeze(accuracy))
...
para = neural_network(X, Y, nn_architecture, 0.1, 5000, 100, 1)
