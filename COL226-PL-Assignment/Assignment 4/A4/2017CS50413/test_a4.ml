...
let rec print_def df = match df with
  Simple(l, ty ,r) -> "def " ^ l ^ " = " ^ (print_tree r)
  | _ -> raise Not_implemented
;;
...
hastype g e t;; (* should be false, as the two branches don't have same type *)
...
let t = Tbool;; 
hastype g e t;; (* should return false, as the claimed type of X isn't possible w.r.t. type assumptions G *)
