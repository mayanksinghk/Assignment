...
void ColumnOrder(float *a, float *b, int n, int m){
    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            a[j*n+i] = b[i*m+j];
        }
...
int main(int argc, char **argv){
    ...
    if(rank == 0){
        ...
        for(int i = 0; i<total; i++){
            ...
            for(int j = 0; j<total; j++){
                if(j == 0){
                    ...
                    MPI_Send(btemp, basic*32, MPI_FLOAT, j, 0, MPI_COMM_WORLD);
                }
            }
            ...
            for(int k = 0; k<total; k++){
                if(k != 0){
                    MPI_Recv(rtemp, basic*basic, MPI_FLOAT, k, k, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  
                }
                for(int l = 0; l<basic; l++){
        ...
        MPI_Recv(atemp,basic*32, MPI_FLOAT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        for(int i = 0; i<total; i++){
            MPI_Recv(btemp, basic*32, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            Mult(atemp, btemp, rtemp, basic, 32, basic);
            MPI_Send(rtemp, basic*basic, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
        }
